...
public class GUIScreenBobmazon extends GuiScreen {
	...
	int getPageCount() {
		...
		return (int) Math.ceil((offers.size() - 1) / 3.0);
	}
	...
	protected void updateButtons() {
		...
		for (int i = currentPage * 3; i < Math.min(currentPage * 3 + 3, offers.size()); i++) {
			...
			if (offer != null) {
				buttons.add(new FolderButton(guiLeft + 34, guiTop + 35 + (54 * (int)Math.floor(i)) - currentPage * 3 * 54, offer));
			}
		}
	...
	protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
		...
		for (int d = currentPage * 3; d < Math.min(currentPage * 3 + 3, offers.size()); d++) {
			Offer offer = offers.get(d);
			if (offer != null) {
				offer.drawRequirement(this, guiLeft + 34, guiTop + 53 + (54 * (int)Math.floor(d)) - currentPage * 3 * 54);
			}
		}
	}
