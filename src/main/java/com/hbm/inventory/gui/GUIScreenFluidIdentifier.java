...
import com.hbm.packet.ItemFluidIDPacket;
...
public class GUIScreenFluidIdentifier extends GuiScreen {
	
    protected static final ResourceLocation texture = new ResourceLocation(RefStrings.MODID + ":textures/gui/gui_fluid_id.png");
    protected int xSize = 143;
    protected int ySize = 175;
    ...
    public GUIScreenFluidIdentifier(EntityPlayer player) {
    ...
    public void initGui()
    {
		...
		this.search = new GuiTextField(0, this.fontRenderer, guiLeft + 50, guiTop + 159, 48, 12);
    ...
    protected void updateButtons() {
        ...
        for(int i = currentPage * 35; i < Math.min(currentPage * 35 + 35, stacks.size()); i++) {
    		buttons.add(new FolderButton(guiLeft + 25 + (18 * (i % 5)), guiTop + 26 + (18 * (int)Math.floor((i / 5D))) - currentPage * 18 * 7, stacks.get(i)));
        }
    ...
    protected void mouseClicked(int i, int j, int k) {
		if(i >= guiLeft + 34 && i < guiLeft + 106 && j >= guiTop + 157 && j < guiTop + 169) {
	...
	protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
		...
		if(search.isFocused())
			drawTexturedModalRect(guiLeft + 34, guiTop + 157, 176, 54, 72, 12);
	...
	class FolderButton {
		...
		public void executeAction() {
			...
			if(type == 0) {
				PacketDispatcher.wrapper.sendToServer(new ItemFluidIDPacket(stack.copy()));
			} else if(type == 1) {
				if(currentPage > 0)
					currentPage--;
				updateButtons();
