...
public class RenderPylon extends TileEntitySpecialRenderer<TileEntityPylon> {
    ...
    public static void renderPowerLines(TileEntityPylonBase pyl, double x, double y, double z) {
        float cableR = pyl.cableColorR / 255.0F;
        float cableG = pyl.cableColorG / 255.0F;
        float cableB = pyl.cableColorB / 255.0F;
        ...
        for (int i = 0; i < pyl.connected.size(); i++) {
            ...
            if(tile instanceof TileEntityPylonBase) {
                ...
                for(int line = 0; line < lineCount; line++) {
                    ...
                    drawLine(first, first.add(new Vec3(mid.xCoord*0.5, mid.yCoord*0.5, mid.zCoord*0.5)), lineCount == 1 ? 0.03125F : 0.055F, mid.length()*0.045, cableR, cableG, cableB);
                }
            }
        }
        GL11.glPopMatrix();
    ...
    public static void drawLine(Vec3 firstPylonMountPos, Vec3 secoundPylonMountPos, float girth, double hang, float cableColorR, float cableColorG, float cableColorB) {
        float count = 10;
        ...
        for(float j = 0; j < count; j++) {
            ...
            drawLineSegment(
                    ...
                    firstPylonMountPos.zCoord + (deltaVector.zCoord * k / count), girth, cableColorR, cableColorG, cableColorB);
        }
    }

    public static void drawLineSegment(double x, double y, double z, double a, double b, double c, float girth, float cableColorR, float cableColorG, float cableColorB) {
