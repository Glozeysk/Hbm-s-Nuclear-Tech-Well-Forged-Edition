...
public class RenderPumpjack extends TileEntitySpecialRenderer<TileEntityMachinePumpjack> {
	...
	public void drawConnection(BufferBuilder buf, double x, double y, double z, double a, double b, double c) {
	...
	public void render(TileEntityMachinePumpjack pj, double x, double y, double z, float f, int destroyStage, float alpha) {
		...
		for(int i = -1; i <= 1; i += 2) {
			drawConnection(buf, 0.53125 * i, 1.5 + rot.yCoord, -5.5 + rot.zCoord + 0.0625D, 0.53125 * i, 3.5 + backPos.yCoord, -3.5 + backPos.zCoord + 0.0625D);
			drawConnection(buf, 0.53125 * i, 1.5 + rot.yCoord, -5.5 + rot.zCoord - 0.0625D, 0.53125 * i, 3.5 + backPos.yCoord, -3.5 + backPos.zCoord - 0.0625D);
		}
		...
		for(int i = -1; i <= 1; i += 2) {
			...
			for(int j = 0; j < 4; j++) {
				...
				drawConnection(buf, (width + pd) * i, 3.5 + sumY1, -3.5 + sumZ1, (width + pd) * i, 3.5 + sumY2, -3.5 + sumZ2);
				drawConnection(buf, (width - pd) * i, 3.5 + sumY1, -3.5 + sumZ1, (width - pd) * i, 3.5 + sumY2, -3.5 + sumZ2);
			}
			...
			drawConnection(buf, (width + pd) * i, 2 + height, 0, (width + pd) * i, 3.5 + sumY, -3.5 + sumZ);
			drawConnection(buf, (width - pd) * i, 2 + height, 0, (width - pd) * i, 3.5 + sumY, -3.5 + sumZ);
		}

		double p = 0.03125D;
		drawConnection(buf, -p, 0.75, -p, -p, height + 1.5, -p);
		drawConnection(buf, p, 0.75,  p, p, height + 1.5, p);
		drawConnection(buf, p, 0.75, -p, p, height + 1.5, -p);
		drawConnection(buf, -p, 0.75, p, -p, height + 1.5, p);
