...
public class TileEntityRBMKControlManual extends TileEntityRBMKControl implements IControlReceiver {
	...
	public void receiveControl(NBTTagCompound data) {
		...
		if(data.hasKey("color")) {
			int c = Math.abs(data.getInteger("color")) % RBMKColor.values().length;
	...
	public void receiveEvent(BlockPos from, ControlEvent e) {
		...
		if (e.name.equals("rbmk_ctrl_set_color")) {
			int colorIndex = (int) (e.vars.get("color").getNumber()) % RBMKColor.values().length;
			if (colorIndex < 0) colorIndex = 0;
			this.color = RBMKColor.values()[colorIndex];
