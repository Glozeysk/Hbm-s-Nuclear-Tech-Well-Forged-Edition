...
public class TileEntityMachineCatalyticCracker extends TileEntityLoadedBase implements IBufPacketReceiver, ITickable, IFluidHandler {
		...
		if(outputFluids != null) {
			...
			while(tanks[0].getFluidAmount() >= 100 && tanks[1].getFluidAmount() >= 200 && hasSpace(outputFluids)) {
				...
				if(outputFluids.length == 2){
					...
					tanks[4].fill(new FluidStack(ModForgeFluids.spentsteam, 2), true);
				} else {
					tanks[2].fill(outputFluids[0].copy(), true);
					tanks[3].fill(new FluidStack(ModForgeFluids.spentsteam, 2), true);
				}
			}
		}
	}
