...
public interface IToolAreaAbility extends IBaseAbility {
    ...
    IToolAreaAbility RECURSION = new IToolAreaAbility() {
        ...
        public boolean onDig(int level, World world, BlockPos pos, EntityPlayer player, ItemToolAbility tool) {
            ...
            if (b == Blocks.STONE && !ToolConfig.recursiveStone) return false;
            if (b == Blocks.NETHERRACK && !ToolConfig.recursiveNetherrack) return false;

            visited.clear();
            recurse(world, pos.getX(), pos.getY(), pos.getZ(), pos.getX(), pos.getY(), pos.getZ(), player, tool, 0, level);
            return false;
        }
        ...
        private void breakExtra(World world, int x, int y, int z, int refX, int refY, int refZ, EntityPlayer player, ItemToolAbility tool, int depth, int radius) {
            if (visited.contains(new ThreeInts(x, y, z))) return;
            depth += 1;
            if (depth > ToolConfig.recursionDepth) return;
            visited.add(new ThreeInts(x, y, z));
            if (x == refX && y == refY && z == refZ) return;
            if (new Vec3d(x - refX, y - refY, z - refZ).length() > radius) return;
            ...
            if (!isSameBlock(b, ref)) return;
            if (meta != refMeta) return;
