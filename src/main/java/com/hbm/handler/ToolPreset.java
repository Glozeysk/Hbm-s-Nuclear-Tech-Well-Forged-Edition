...
public class ToolPreset {
    ...
    public ITextComponent getMessage() {
        if (isNone()) {
            return ChatBuilder.start("[" + I18n.format("chat.abildisabled") + "]").color(TextFormatting.GOLD).flush();
        }
        ...
        if (hasArea && hasHarvest) builder.next(" + ");
        if (hasHarvest) {
    ...
    public void restrictTo(com.hbm.handler.ability.AvailableAbilities availableAbilities) {
        int maxAreaLevel = availableAbilities.maxLevel(areaAbility);
        if(maxAreaLevel == -1) { areaAbility = IToolAreaAbility.NONE; areaAbilityLevel = 0; }
        else if(areaAbilityLevel > maxAreaLevel) areaAbilityLevel = maxAreaLevel;
        else if(areaAbilityLevel < 0) areaAbilityLevel = 0;
        ...
        if(maxHarvestLevel == -1) { harvestAbility = IToolHarvestAbility.NONE; harvestAbilityLevel = 0; }
        else if(harvestAbilityLevel > maxHarvestLevel) harvestAbilityLevel = maxHarvestLevel;
        else if(harvestAbilityLevel < 0) harvestAbilityLevel = 0;
    }
}