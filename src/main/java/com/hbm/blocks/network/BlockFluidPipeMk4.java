...
public class BlockFluidPipeMk4 extends BlockContainer implements IToolable, ILookOverlay {
	...
	public TileEntity createNewTileEntity(World worldIn, int meta) {
		if(meta > 0){
			return new TileEntityFFFluidSuccMk4();
		} else {
	...
	public boolean onScrew(World world, EntityPlayer player, int x, int y, int z, EnumFacing side, float fX, float fY, float fZ, EnumHand hand, ToolType tool){
		if(tool == ToolType.SCREWDRIVER){
			...
			world.setBlockState(pos, ModBlocks.fluid_duct_mk4.getDefaultState().withProperty(BlockFluidPipeMk2.EXTRACTS, !extracts));
