...
public class BlockFluidPipeMk3Solid extends BlockContainer implements IToolable, ILookOverlay {
	...
	public TileEntity createNewTileEntity(World worldIn, int meta) {
		if(meta > 0){
			return new TileEntityFFFluidSuccMk3Solid();
		} else {
			return new TileEntityFFFluidDuctMk3Solid();
		}
	}
	...
	public boolean onScrew(World world, EntityPlayer player, int x, int y, int z, EnumFacing side, float fX, float fY, float fZ, EnumHand hand, ToolType tool){
		if(tool == ToolType.SCREWDRIVER){
			...
			boolean extracts = state.getValue(BlockFluidPipeMk3Solid.EXTRACTS);
			world.setBlockState(pos, this.getDefaultState().withProperty(BlockFluidPipeMk3Solid.EXTRACTS, !extracts));
